...
<script>
  window.addEventListener('DOMContentLoaded', function(){
    ...
    // すでにiframeがあるか確認（重複防止）
    if (document.getElementById('dify-chatbot-bubble-window')) return;

    const state = localStorage.getItem("chatWidgetOpen");
    if (state !== "block") return;  // 前回閉じていたなら何もしない

    const params = new URLSearchParams(config.inputs || {}).toString();
    const src = `${config.baseUrl || `https://${config.isDev ? "dev." : ""}udify.app`}/chatbot/${config.token}?${params}`;
    ...
    if (button) {
      ...
      if (btnContent) {
        btnContent.innerHTML = `<img id="closeIcon" src="naki.png" alt="ニコちゃん泣き" style="width:100%; height:100%;">`;
      }
    }
  });
</script>
