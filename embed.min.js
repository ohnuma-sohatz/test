(()=>{let n="difyChatbotConfig",a="dify-chatbot-bubble-button",c="dify-chatbot-bubble-window",p=window[n],h={open: `<img id="openIcon" src="nicochan.png" alt="ニコちゃん" style="width:100%; height:100%;">
			...
			</svg>`,close: `<img id="closeIcon" src="naki.png" alt="ニコちゃん泣き" style="width:100%; height:100%;">
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    const checkInterval = setInterval(() => {
        ...
        if(button){
            ...
            const state = localStorage.getItem("chatWidgetOpen");
            let widget = document.getElementById('dify-chatbot-bubble-window');

            if(state === 'block' && !widget){
                // iframe（チャット画面）を直接生成する
                widget = document.createElement('iframe');
                widget.allow = "fullscreen;microphone";
                widget.title = "dify chatbot bubble window";
                widget.id = 'dify-chatbot-bubble-window';
                
                const params = new URLSearchParams(config.inputs || {}).toString();
                widget.src = `${config.baseUrl || `https://${config.isDev?"dev.":""}udify.app`}/chatbot/${config.token}?${params}`;
                ...
                if(buttonContent){
                    buttonContent.innerHTML = `<img id="closeIcon" src="naki.png" alt="ニコちゃん泣き" style="width:100%; height:100%;">`;
                }
            }
        }
    }, 100); // 0.1秒間隔でボタンの生成を待つ
});
